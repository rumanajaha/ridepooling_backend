...
const createRide = async (req, res, next) => {
  try {
    let rideData = {
      ...
      seatsAvailable: req.body.seatsTotal,
    };
...
const listRides = async (req, res, next) => {
  try {
    ...
    if (destination)
      query.$text = {
        ...query.$text,
        $search: `${query.$text?.$search || ""} ${destination}`,
      };
...
const searchRides = async (req, res, next) => {
  try {
    ...
    if (!lat || !lng) {
      throw Object.assign(new Error("Latitude and longitude are required"), {
        status: 400,
        code: "MISSING_FIELDS",
      });
    }
...
const getRide = async (req, res, next) => {
  try {
    const ride = await Ride.findById(req.params.id)
      .populate("driverId", "name rating avatarUrl vehicle")
      .lean();
    if (!ride)
      throw Object.assign(new Error("Ride not found"), {
        status: 404,
        code: "NOT_FOUND",
      });
...
const updateRide = async (req, res, next) => {
  try {
    ...
    if (ride.status !== "open")
      throw Object.assign(new Error("Cannot update non-open ride"), {
        status: 403,
        code: "INVALID_STATUS",
      });
    ...
    if (
      updates.origin &&
      (!ride.originCoords || updates.origin !== ride.origin)
    ) {
      updates.originCoords = await geocodeLocation(updates.origin);
    }
    if (
      updates.destination &&
      (!ride.destinationCoords || updates.destination !== ride.destination)
    ) {
...
const deleteRide = async (req, res, next) => {
  try {
    ...
    if (bookings > 0)
      throw Object.assign(
        new Error("Cannot delete ride with active bookings"),
        { status: 403, code: "ACTIVE_BOOKINGS" }
      );
