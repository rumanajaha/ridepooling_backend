...
const register = async (req, res, next) => {
  try {
    ...
    if (existingUser) throw Object.assign(new Error('User already exists'), { status: 400, code: 'USER_EXISTS' })
...
const login = async (req, res, next) => {
  try {
    ...
    if (!email || !password) {
      throw Object.assign(new Error('Email and password are required'), { status: 400, code: 'MISSING_FIELDS' })
    }

    const user = await User.findOne({ email })
    if (!user) {
      throw Object.assign(new Error('Invalid credentials'), { status: 400, code: 'INVALID_CREDENTIALS' })
    }
